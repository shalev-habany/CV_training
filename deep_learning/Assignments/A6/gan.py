...
def discriminator_loss(logits_real, logits_fake):
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    y = torch.ones_like(logits_real)
    y_fake = torch.zeros_like(logits_fake)
    loss_real = torch.nn.functional.binary_cross_entropy_with_logits(logits_real, y, reduction='mean')
    loss_fake = torch.nn.functional.binary_cross_entropy_with_logits(logits_fake, y_fake, reduction='mean')
...
def generator_loss(logits_fake):
    ...
    loss = torch.nn.functional.binary_cross_entropy_with_logits(logits_fake, y, reduction='mean')
    ##############################################################################
    #                              END OF YOUR CODE                              #
    ##############################################################################
    return loss
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    y = torch.ones_like(scores_real)
    y_fake = torch.zeros_like(scores_fake)
    loss_real = torch.mean((scores_real - y) ** 2)
    loss_fake = torch.mean((scores_fake - y_fake) ** 2)
...
def ls_generator_loss(scores_fake):
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    y = torch.ones_like(scores_fake)
    loss = 0.5 * torch.mean((scores_fake - y) ** 2)
    ##############################################################################
    #                              END OF YOUR CODE                              #
    ##############################################################################
    return loss
