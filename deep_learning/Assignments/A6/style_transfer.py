...
def content_loss(content_weight, content_current, content_original):
    ...
    flattened_current = content_current.view(content_current.size(1), -1)
    flattened_original = content_original.view(content_original.size(1), -1)
    content_loss = content_weight * \
        nn.functional.mse_loss(
            flattened_current, flattened_original, reduction='sum')
    return content_loss
    ############################################################################
    #                               END OF YOUR CODE                           #
    ############################################################################
...
def gram_matrix(features, normalize=True):
    ...
    ############################################################################
    # Replace "pass" statement with your code
    flattened_features = features.view(features.size(0), features.size(1), -1)
    gram = flattened_features @ flattened_features.transpose(1, 2)
    if normalize:
        gram /= (features.size(1) * features.size(2) * features.size(3))
        ############################################################################
        #                               END OF YOUR CODE                           #
        ############################################################################
    return gram
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i in range(len(style_layers)):
        ...
        loss = weight * \
            nn.functional.mse_loss(current_gram, target, reduction='sum')
        style_loss += loss
    return style_loss
...
def guided_gram_matrix(features, masks, normalize=True):
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    guided_gram = torch.zeros(features.size(0), features.size(1),
                              features.size(2), features.size(2)).to(features.device)
    flattened_features = features.view(features.size(0), features.size(1), -1)
    guided_flattened_features = torch.zeros_like(flattened_features).to(features.device)
    for i in range(features.size(0)):
        guided_flattened_features[:, i] = flattened_features[i] * \
            masks[i].view(features.size(1), -1)
    guided_gram = guided_flattened_features @ \
        guided_flattened_features.transpose(1, 2)
    if normalize:
        guided_gram /= (features.size(1) * features.size(2) * features.size(3))
    return guided_gram
